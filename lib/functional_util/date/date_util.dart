...
class DateFormatUtil {
  ...
  static String formatDate(DateFormat dateFormat, String time) {
    return time.isEmpty ? '' : dateFormat.format(DateTime.parse(time));
  }
  ...
  static DateTime makePaymentDateParser(String date) {
    final val = utcTimeFormat.format(DateTime.parse(date));
  ...
  // static String makePaymentExpTimer(DateTime date) {
  //   final now = DateTime.now();
  //   Log().debug('Compare ', date.hour == now.hour);

  //   if (now.hour == date.hour && now.minute < date.minute) {
  //     return '';
  //   }
  //   Log().debug('now $now ', date);
  //   return 'Expired';
  // }
}
...
final dateMonthTimeFormat2 = DateFormat('MMM d. At hh:mm a');
