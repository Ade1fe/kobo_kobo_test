...
class BillPinArgs {
  ...
  BillPinArgs({required this.routeTo});
  factory BillPinArgs.empty() {
    return BillPinArgs(
      routeTo: '',
    );
  }
...
class BillPinScreen extends StatefulWidget {
  const BillPinScreen({super.key, required this.args});
  final BillPinArgs args;
            ...
            AppButton(
              ...
              action: () {
                AppNavigator.of(context).push(
                  widget.args.routeTo,
                  args: TransferPinArgs(route: AppRoutes.topUp),
                );
                if (mounted) {
                  vm.showBtmSheet = true;
                }
              },
            ),
            YBox(Insets.dim_34.h),
